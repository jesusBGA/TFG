...
sql9 =  "SELECT distinct C.date, C.M FROM caelis.cml_curvature C JOIN caelis.cml_aod A ON (A.ph=C.ph && A.station= %s && A.date=C.date) WHERE (C.ph= %s && C.M is not null && C.date between %s and %s && (A.cloud_screening_v3!= 'null_values' && A.cloud_screening_v3!= 'notL10')) GROUP BY C.date;"
sql91 = "SELECT distinct C.date, C.M FROM caelis.cml_curvature C JOIN caelis.cml_aod A ON (A.ph=C.ph && A.station= %s && A.date=C.date) WHERE (C.ph= %s && C.M is not null && C.date between %s and %s && (A.cloud_screening_v3= 'cloud_free' || A.cloud_screening_v3= 'restoration')) GROUP BY C.date;"
